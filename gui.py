...
def display_row_in_box(flight_line, airline_name, airline_code, equipment,  origin_code,origin_city,destination_code, destination_city, altitude,climb_descend, distance,direction,flight_heading):
    ...
    if(len(flight_line.children)>0):
        ...
        wi_airline_logo = Picture(airline_box, image =airline_logo,align="top",width=40,height=40)
        wi_airline_name = Text(airline_box, text=airline_name,align="top", size=12)
        wi_airline_code = Text(flight_line, text=airline_code, grid=[2,0],width=8, size=12)
        wi_equipment = Text(flight_line, text=equipment, grid=[4,0],width=6, size=14)
        Text(flight_line, text=" ", grid=[5,0])
        origin_box = Box(flight_line,border=False,height=90,width=180,grid=[6,0])
        Text(origin_box, text=" ", align="top", size=12)
        wi_origin_name = Text(origin_box, text=origin_code, align="top", size=14)
        wi_origin_city = Text(origin_box, text=origin_city, align="top", size=12)
        destination_box = Box(flight_line,border=False,height=90,width=180,grid=[10,0])
        Text(destination_box, text=" ", align="top", size=12)
        wi_destination_name = Text(destination_box, text=destination_code, align="top", size=14)
        wi_destination_city = Text(destination_box, text=destination_city, align="top", size=12)
        wi_heading = Picture(flight_line, image='./images/' + flight_heading + '.png', height=35, width=40, grid=[15, 0])
        wi_altitude = Text(flight_line, text=f'{round(altitude,-2):,}' , grid=[16, 0], width=8, size=14)
        wi_climb_descend = Picture(flight_line, image ='./images/'+climb_descend+'.png',height=35,width=35, grid=[18,0])
        Text(flight_line, text="", grid=[20, 0])
        wi_distance_direction = Text(flight_line, text=str(distance),width=10, grid=[22,0], size=14)
...
if __name__ == '__main__':
    ...
    app = App(title="Nearby Flights", height=700, width=1024)
    app.bg = "white"
    flight_line_1 = Box(app, layout="grid", border=False, height=120, width=1000, align="top")
    #padding_box_1 = Box(app, align="top", width="fill", height=2)
    flight_line_2 = Box(app, layout="grid", border=False, height=120, width=1000, align="top")
    #padding_box_2 = Box(app, align="top", width="fill", height=2)
    flight_line_3 = Box(app, layout="grid", border=False, height=120, width=1000, align="top")
    #padding_box_3 = Box(app, align="top", width="fill", height=2)
    flight_line_4 = Box(app, layout="grid", border=False, height=120, width=1000, align="top")
    #padding_box_4 = Box(app, align="top", width="fill", height=2)
    flight_line_5 = Box(app, layout="grid", border=False, height=120, width=1000, align="top")
    #padding_box_5 = Box(app, align="top", width="fill", height=2)
    flight_line_6 = Box(app, layout="grid", border=False, height=120, width=1000, align="top")
