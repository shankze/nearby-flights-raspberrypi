...
call_list = []
skip_airline_list = ['LIF','EJA','JSX','FFL','LXJ','DVY','EJM','JRE','JTL','FWK','KII']
...
def get_airport_info_from_json(location_code):
    with open('data/airports.json', 'r', encoding='utf8') as f:
...
def filter_flights_on_opensky_data(flights, location_info):
    filtered_flights = []
    for flight in flights:
...
def get_from_opensky(flight_aware_cache_dict, config):
    ...
    flights = fetch_nearby_flights_from_opensky_api(config, location_info)
    if flights is None:
        return []
    filtered_flights = filter_flights_on_opensky_data(flights, location_info)
    sortedFlights = sorted(filtered_flights, key =lambda x:x['distance'])
    flight_data = []
    for flight in sortedFlights:
        try:
        ...
        if ('icao' in location_info) and ('FilterByDestination' in config) and ('FilterByOrigin' in config) and (config['FilterByDestination'] == True) and (config['FilterByOrigin'] == True):
            if (airline_details_from_fa['destination'] != location_info['icao']) and (airline_details_from_fa['origin'] != location_info['icao']):
                # only if origin or destination is the said airport
                continue
        else:
            if ('icao' in location_info) and ('FilterByDestination' in config) and (config['FilterByDestination']==True) and (airline_details_from_fa['destination'] != location_info['icao']):
                print('Skipping')
                continue
            if ('icao' in location_info) and ('FilterByOrigin' in config) and (config['FilterByOrigin']==True) and (airline_details_from_fa['origin'] != location_info['icao']):
                print('Skipping')
                continue
        if airline_details_from_fa['airline_name'] == None:
...
def fetch_nearby_flights_from_opensky_api(config, location_info):
